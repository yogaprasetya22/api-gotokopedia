...
func TestRateLimiterMiddleware(t *testing.T) {
	...
	for i := 0; i < cfg.rateLimiter.RequestsPerTimeFrame+marginOfError; i++ {
		...
		if err != nil {
			t.Fatalf("tidak dapat membuat permintaan: %v", err)
		}
		...
		if err != nil {
			t.Fatalf("tidak dapat mengirim permintaan: %v", err)
		}
		defer resp.Body.Close()

		if i < cfg.rateLimiter.RequestsPerTimeFrame {
			if resp.StatusCode != http.StatusOK {
				t.Errorf("status yang diharapkan ok; telah mendapatkan %v", resp.Status)
			}
		} else {
			if resp.StatusCode != http.StatusTooManyRequests {
				t.Errorf("status yang diharapkan terlalu banyak permintaan; telah mendapatkan %v", resp.Status)
			}
		}
	}
}
