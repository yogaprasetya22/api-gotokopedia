...
func (s *UserStore) CreateByOAuth(ctx context.Context, user *User) error {
	query := `
		INSERT INTO users (google_id, picture, username, email, is_active, role_id) VALUES
	($1, $2, $3, $4, $5, (SELECT id FROM roles WHERE name = $6))
	RETURNING id, created_at
	`
...
func (s *UserStore) GetByGoogleID(ctx context.Context, googleID string) (*User, error) {
	query := `
		SELECT id, google_id, picture, username, email, created_at, role_id, is_active FROM users WHERE google_id = $1 and is_active = true
	`
	...
	user := &User{}
	err := s.db.QueryRowContext(ctx, query, googleID).Scan(&user.ID, &user.GoogleID, &user.Picture, &user.Username, &user.Email, &user.CreatedAt, &user.RoleID, &user.IsActive)
